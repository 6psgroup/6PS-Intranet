		...
		<tbody id="the-list">
			{section name=catTree loop=$catTree}
				<tr id='post-{$catTree[catTree].id}' class='{cycle values="alternate2,alternate"}'>
					...
					<td nowrap="nowrap">{$catTree[catTree].name}</td>
					<td class="href_noUnderline" style="text-align:center;" nowrap="nowrap">
						{if $catTree[catTree].navUp == true}
							<a href="/intranet/module/Assets/catSortUp.php?{$catTree[catTree].id}">
						...
						{if $catTree[catTree].navDown == true}
							<a href="/intranet/module/Assets/catSortDown.php?{$catTree[catTree].id}">
					...
					<td style="text-align:center"><a href="/intranet/module/Assets/catEdit.php?{$catTree[catTree].id}">Edit</a></td>
					<td style="text-align:center"><a href="/intranet/module/Assets/catDelete.php?{$catTree[catTree].id}" onClick="return confirm('Are you sure you wish to delete this category (and all items underneith it)?');">Delete</a></td>
				</tr>
			{/section}
		</tbody>
	</table>
